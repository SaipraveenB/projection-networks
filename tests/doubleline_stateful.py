...
from models.spnn import StatefulACPNN
...
testenv.add_shapes( Line( np.asarray([-1,-1]), np.asarray([0,0]), np.asarray([1,0,0,1])) )
testenv.add_shapes( Line( np.asarray([1,-1]), np.asarray([0,0]), np.asarray([0,1,0,1])) )
...
sfpnn.fit(train_states, train_uvs, train_obs, epochs=1000)
