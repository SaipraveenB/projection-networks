...
attns = pnn.predict_attention(test_acs, test_uvs)
...
attns = attns.reshape([attns.shape[0], N_ - N_split, W, attns.shape[2]]).transpose([1,0,2,3])

plotter = PyGame2D()
plotter.draw_path_cmp(testenv, outputss[-1], preds[-1], attns[-1])
