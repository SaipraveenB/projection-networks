...
testenv.add_shapes( Circle( 1, np.asarray([0,0]), np.asarray([1,0,0,1])) )
...
states = np.concatenate([poss, dirs])
...
pnn.fit(train_acs, train_uvs, train_obs, epochs=200)

# TxNxC
preds = pnn.predict(train_acs, train_uvs)
...
# Unwrap preds.
# N_xTxWxC
preds = preds.reshape([preds.shape[0], N_split, W, preds.shape[2]]).transpose([1,0,2,3])
